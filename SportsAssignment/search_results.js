const outputData = localStorage.getItem('inputData');
document.getElementById('outputData').textContent = outputData ? outputData : 'No data available.';


// const p=document.getElementById("inputData")

const loadAllPlayer= ()=>{
    fetch(`https://www.thesportsdb.com/api/v1/json/3/searchplayers.php?p=${outputData}`)
    .then(res=>res.json())
    .then(data=>{
        ...
        else displayPlayer(data.player);
    })
};
...
const nodata=()=>{
    ...
    console.log(div);
    div.innerHTML=`
      <h1 class="error"><b>No data Found</b></h1>
    `;
}
